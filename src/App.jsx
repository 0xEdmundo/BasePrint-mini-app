...
const App = () => {
  ...
  // External data state
  const [neynarProfile, setNeynarProfile] = useState(null);
  ...
  // Fetch external data when address changes
  useEffect(() => {
    if (!address) return;
    // Fetch Neynar profile (using a placeholder FID for demo)
    const fetchNeynar = async () => {
      setNeynarLoading(true);
      try {
        const res = await fetch(`/api/neynar?fid=12345`);
        const json = await res.json();
        if (json.success && json.user) {
          setNeynarProfile(json.user);
        } else {
          console.error('Neynar error:', json.error);
        }
      } catch (e) {
    ...
    // Fetch native balance via Etherscan
    const fetchBalance = async () => {
      ...
      try {
        ...
        const json = await res.json();
        if (json.success) {
          setNativeBalance(json.balance);
        } else {
          console.error('Etherscan error:', json.error);
        }
      } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans">
      <div className={cardClass}>
        <h1 className={`text-4xl font-extrabold ${baseBlue} mb-2 text-center`}>BasePrint</h1>
          ...
          <div className="mt-4 text-sm text-gray-700">
            ...
            {neynarProfile ? (
              <div className="flex items-center space-x-2">
                <img src={neynarProfile.pfp_url} alt="avatar" className="w-8 h-8 rounded-full" />
                <span>{neynarProfile.display_name || neynarProfile.username}</span>
              </div>
            ) : neynarLoading ? (
              <span>Loading Farcaster profile...</span>
            ) : (
              <span>No Farcaster profile loaded.</span>
            )}
            {/* Native balance */}
            {nativeBalance !== null ? (
              <div>Balance: {nativeBalance} ETH</div>
            ) : balanceLoading ? (
        ...
        <div className="mb-6 p-4 bg-gray-50 border border-blue-200 rounded-lg text-sm">
          ...
          {isWalletConnected && (
            <>
              ...
              <p className="font-mono break-all bg-gray-100 p-2 rounded text-xs border border-gray-200">{address}</p>
              <p className="font-medium text-gray-700 mt-2">Chain ID:</p>
              <p className="font-mono bg-gray-100 p-2 rounded text-xs border border-gray-200">{chainId ? chainId : 'Unknown'}</p>
            </>
          )}
        </div>
        {/* Error Message */}
        ...
        <div className="space-y-4 pt-2">
          {!isWalletConnected && (
            <button onClick={connectWallet} className={buttonClass + ' w-full'}>
          ...
          {isWalletConnected && message.includes('NFT successfully minted') && (
            <button onClick={castToFarcaster} className={buttonClass + ' w-full bg-purple-600 hover:bg-purple-700'} disabled={isCasting}>
              {isCasting ? (
                <>
                  <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>
                  Casting...
